(async function () {
  ...
  try {
    const manifestUrl = `${repoBase}/data/session_viewer/manifest.json`;
    ...
    showContent();
    if (typeof SessionViewerApp !== "undefined" && typeof SessionViewerApp.mount === "function") {
      await SessionViewerApp.mount(document.getElementById("session-viewer-content"), characterData, cfg);
    } else {
  ...
  function installBaseShellStyles() {
    ...
    style.textContent = `
      *, *::before, *::after { box-sizing: border-box; }
      ...
      #session-viewer-content { min-height: 100vh; }
    `;
  ...
  function renderLoadingShell() {
    root.innerHTML = `
      ...
      <div id="state-fatal" class="shell-state" hidden>
        <div class="shell-card shell-stack">
          <h1>This session file is out of date</h1>
      ...
      <main id="session-viewer-content" hidden></main>
    `;
  }
  ...
  async function loadLibraryFromManifest(libCfg) {
    ...
    try {
      ...
      console.warn("Session View library load failed:", error);
