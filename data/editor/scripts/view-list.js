...
const CharacterList = (() => {
      ...
      hpDisplay = `
        <div class="card-hp">
          ${typeof HpWidgets !== "undefined"
            ? HpWidgets.renderBar(hp, {
                variant: "editor",
                wrapperClass: "",
                trackClass: "hp-bar-track",
                fillClassBase: "hp-bar-fill",
                fillTag: "div",
                readout: false,
              })
            : `<div class="hp-bar-track"><div class="hp-bar-fill" style="width:0%"></div></div>`}
  ...
  function wireCardEl(cardEl, filePath, sha) {
    ...
    cardEl.querySelector(".btn-export-sheet")?.addEventListener("click", async () => {
      const result = await App.loadCharacter(filePath);
      if (!result) return;
      SheetExporter.openExportChooser(result.data, filePath);
    });
  ...
  async function openSheetPreview(characterData, filePath) {
    ...
    overlay.querySelector("#btn-export-preview").addEventListener("click", () => {
      SheetExporter.openExportChooser(characterData, filePath);
    });
