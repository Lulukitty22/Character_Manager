...
const LibraryRecords = (() => {
  ...
  function recordPath(record = {}, fallbackCollection = "") {
    ...
    if (id.startsWith("tag:")) return `library/records/tags/${slugify(id.slice(4))}.json`;
    ...
    return `library/records/${deepest}/${fileName}.json`;
  }

  function indexPath(collection = "", path = "") {
    const clean = path.replace(/^library\/records\/|\/[^/]+\.json$/g, "");
    return clean ? `library/records/${clean}/index.json` : `library/records/${canonicalCollection(collection)}/index.json`;
  }
