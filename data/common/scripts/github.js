 ...
 *   githubToken  â€” Personal Access Token with repo scope
 *   githubOwner  â€” Repository owner (username or org)
 *   githubRepo   â€” Repository name
 *   githubBranch â€” Branch to read/write (default: "staging" while this
...
const GitHub = (() => {
  ...
  const CHARACTER_ROOT = "library/characters";
  ...
  /**
   * List all files in the library/characters/ directory.
  ...
  /**
   * List JSON files in a library subfolder, e.g. library/records/items.
   */
  ...
  async function writeLibraryFile(fileName, data, sha = null) {
    return writeCharacterFile(`${LIBRARY_ROOT}/${fileName}`, data, sha, `Update ${LIBRARY_ROOT}/${fileName}`);
  }
   ...
   *
   * @param {string} filePath   e.g. "library/characters/capella.json"
  ...
  async function verifyConfig() {
    ...
    try {
      ...
      if (response.status === 401) return { ok: false, error: "Invalid token â€” authentication failed." };
